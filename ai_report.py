...
def generate_parent_friendly_report(analysis: dict[str, Any], api_key: str) -> str:
    ...
    rules = [
        ...
        "3. 不给具体买入、卖出、加仓、减仓指令。",
        "4. 专业词汇保留原名，后面用括号加一句通俗解释，例如：换手率（今天买卖这只股票的人特别多）、ROE（公司用自己的钱赚钱的能力）、负债率（公司借了多少钱相对自己的家底）、净利率（卖出去的东西最终能留下多少利润）、毛利率（产品本身的赚钱空间有多大）、营收增长率（公司收入有没有在增加）、净利润增长率（公司最终赚到的钱有没有在增加）。",
